...
---
 ...
 The application uses **MongoDB** with **Mongoose** for schema definition, validation, relationships, and database operations.

 The data model is organized around the following primary entities:
...
---
 ...
 Represents published or unpublished video content.
...
---
 ...
 Stores comments associated with videos and their authors.
...
---
 ...
 Represents user interactions with supported resources.
...
```
 ...
 The data model can enforce uniqueness so that a user cannot create duplicate likes for the same resource.

```
User + Video = Unique Like
```
...
---
 ...
 Represents a subscription relationship between users/channels.
...
- Creator audience information

---

 ## 📂 Playlist

 Allows users to organize videos into collections.
...
| User → Watch History | One-to-Many |
| User ↔ Channel | Many-to-Many through Subscriptions |
| Video → Comment | One-to-Many |
| User ↔ Video | Many-to-Many through Likes |
| Playlist ↔ Video | Many-to-Many |
| User ↔ Video | Many-to-Many through Watch History |
